...
// fin atributos importantes
			...
			<div class="col-lg-6">
				...
				if ($key['tags']['allow_variations']=='true') {
					...
					$comparar= array_column($verVariacionCreado, 'attribute_combinations');
					for ($i=0; $i < count($comparar); $i++) { 
						for ($i2=0; $i2 < count($comparar[$i]); $i2++) { 
							$comparar2[]= $comparar[$i][$i2]['value_name'];
						}
					}
					foreach ($key['values'] as $key2) {
						if (in_array($key2['name'], $comparar2)) {
							$chequeado= 'checked';
						}
						else { $chequeado= '';}
				?>
					<input type="hidden" name="id" value="<?php echo $verVariacionCreado['id']; ?>">
					<input type="hidden" name="V-<?php echo $key['id']; ?>" value="variationOK">
					<br><input <?php echo $chequeado; ?> type="checkbox" value="<?php echo $key2['name']; ?>" name="<?php echo $key['id']; ?>[]"> <?php echo $key2['name']; ?> <b>Url Imagen:</b> <input style="width: 160px;" type="text" name="<?php echo $key['id']; ?>-imgVariacion" value="<?php echo $imagen[0]; ?>">
				<?php
					}
				}

...
for ($p = 0; $p < $paginas; $p++) {
    ...
    foreach ($articulos as $articulo) {
        ...
        $data['id'] = '856583-MLA26835125955_022018';
        $desc = $meli->post('items/'.$item.'/pictures?access_token='.$token,$data);
            // $descrip = array('plain_text' => $articulo['header'].$articulo['description'].$articulo['footer']);
...
function articulosAdmin($offset, $porPagina, $filtro)
{
    ...
    $query = "SELECT idML, idEmpresa, ac_token,header,footer, title, category_id, price, available_quantity, warranty, siteML, pictures, idART, description FROM articulosML inner join user_config on id_admin = idEmpresa WHERE idML != '' AND idML is not null $where limit $offset, $porPagina;";
    $stmt = $link->prepare($query);
    $stmt->execute();
    $resultado = $stmt->fetchAll(\PDO::FETCH_ASSOC);
    return $resultado;
...
function insertar($tabla, $datos)
{
    ...
    $link = new \PDO(
        "mysql:host=single-priva12.privatednsorg.com;dbname=mconnect_basegeneral",

...
function articulosAdmin($offset, $porPagina, $filtro)
{
    ...
    $query = "SELECT idML, idEmpresa, ac_token, title, category_id, price, available_quantity, warranty, siteML, pictures, idART, description FROM articulosML inner join user_config on id_admin = idEmpresa WHERE idML != '' AND idML is not null $where limit $offset, $porPagina;";
    $stmt = $link->prepare($query);
    $stmt->execute();
    $resultado = $stmt->fetchAll(\PDO::FETCH_ASSOC);
    return $resultado;

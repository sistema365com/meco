...
if ($presta['presta']==0) {
	...
	echo '<input class="form-control" type="text" name="pictures" placeholder="Agregar urls de imagenes separadas por punto y coma ( ; )" value="'.$url['urlImg'].$_GET['id'].'.jpg">';
}
if ($presta['presta']==1) {
echo '<input id="inputIMG" type="hidden" name="pictures" value="">';
$url= configuracion::verConfigu();
if ($url['configIMGPRESTA']==1) {
$listarImg= $objArticulo->listarImgPresta($_GET['id']);
}
if ($url['configIMGPRESTA']==2) {
$listarImg= $objArticulo->listarImgPresta2($_GET['id']);
}
$i= 0;

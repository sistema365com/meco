...
if ($borrarRegistro) {
        ...
        echo "<script>alert('No puede eliminarse a usted mismo'); document.location=('../usuarios.php?s=verUsuarioid=".$id."&alerta=error')</script>";
        exit();
        }
        $borrar= $objNota->borrarUser($idNotaGET,$_SESSION['prefijo']);
        if ($borrar) {
...
// validaciones
	if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
		echo "<script>alert('El correo ingresado no es una direccion valida'); document.location=('../usuarios.php?s=verUsuarioid=".$id."&alerta=error')</script>";
		exit();
	}
	// if ($validar==1) {
	// 	echo "<script>alert('El correo electronico ya se encuentra registrado, recupera tu contraseña'); document.location=('../usuarios.php?s=verUsuarioid=".$id."&alerta=error')</script>";
	// 	exit();
	// }
	if ($pass != $pass2) {
		echo "<script>alert('Las contraseñas ingresadas deben ser iguales'); document.location=('../usuarios.php?s=verUsuarioid=".$id."&alerta=error')</script>";
		exit();
	}
	if (!$nombre) {
		echo "<script>alert('Falta completar el Nombre'); document.location=('../usuarios.php?s=verUsuarioid=".$id."&alerta=error')</script>";
		exit();
	}
	if (!$email) {
		echo "<script>alert('Falta completar el Correo electronico'); document.location=('../usuarios.php?s=verUsuarioid=".$id."&alerta=error')</script>";
		exit();
	}
...
if (!$nuevoRegistro) {
    ...
    if ($edita) {
        ...
        header('Location: ../usuarios.php?alerta=editado');
        }
        else
...
elseif ($nuevoRegistro) {
    ...
    if ($crear) {
        ...
        header('Location: ../usuarios.php?alerta=creado');
    }
    else { echo "error de carga";}
}
